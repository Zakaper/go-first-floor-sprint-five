...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	averageSpeedRunning := meanSpeed(action, duration)
	return ((runningCaloriesMeanSpeedMultiplier * averageSpeedRunning * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	height = height / 100
	averageSpeedWalking := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(averageSpeedWalking, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
